...
class featureFinder():
    ...
    def id_words_counter(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        dataframe['mean_words_emoji_sent'] = dataframe['body'].str.split(' ').apply(lambda x: len(x))
        if 'emoticons' in dataframe.columns:
            dataframe['mean_emoji_sent'] = dataframe["emoticons"].apply(lambda x: 0 if type(x) == float else len(x))
        else:
            dataframe['mean_emoji_sent'] = 0.0
        dataframe['mean_words_sent'] = dataframe['mean_words_emoji_sent'] - dataframe['mean_emoji_sent']

        dataframe = dataframe.astype({"_id": str, "mean_words_emoji_sent": int, "mean_emoji_sent": int, "mean_words_sent":int})

        # find the top X users who sent the most number of words/emoji/both
        ids_sum_spam = dataframe.groupby('_id').sum()
        
        self.top_id_words_emoji = self.id_categorizer(ids_sum_spam, 'mean_words_emoji_sent',top=5)
        ...
        dataframe['num_chats_by_top_emoji_users'] = dataframe['_id'].apply(lambda x: True if x in self.top_id_emoji else False)
        dataframe['num_chats_by_top_words_users'] = dataframe['_id'].apply(lambda x: True if x in self.top_id_words else False)
        dataframe['num_chats_by_top_words_emoji_users'] = dataframe['_id'].apply(lambda x: True if x in self.top_id_words_emoji else False)
    ...
    def clean_dataframe(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        dataframe = dataframe[['start','end','num_chats_by_top_emoji_users', 'num_chats_by_top_words_users', 'num_chats_by_top_words_emoji_users']]
        return dataframe